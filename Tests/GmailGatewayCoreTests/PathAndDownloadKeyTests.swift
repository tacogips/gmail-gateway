...
@Test func isWithinRootAcceptsRootAndDescendants() throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("gmail-gateway-root-\(UUID().uuidString)", isDirectory: true)
    defer {
...
@Test func isWithinRootRejectsTraversalAndSiblingPrefix() throws {
    let base = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("gmail-gateway-roots-\(UUID().uuidString)", isDirectory: true)
...
@Test func isWithinRootRejectsSymlinkEscape() throws {
    let base = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("gmail-gateway-symlink-\(UUID().uuidString)", isDirectory: true)
    let root = base.appendingPathComponent("allowed", isDirectory: true)
