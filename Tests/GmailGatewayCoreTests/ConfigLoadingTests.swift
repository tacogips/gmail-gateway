...
@Test func configLoadRejectsCredentialEnvironmentSuffixCollisions() throws {
    ...
    let error = try requireGmailGatewayError {
        _ = try GmailGatewayConfigLoader.loadConfig(configPath: paths.root + "/config.toml", environment: [:])
    }
...
@Test func cachePruneDoesNotRequireUnreadableOAuthClientSecret() throws {
    ...
    let pruneResult = GmailGatewayCLI().run(
        arguments: ["cache", "prune", "--config", paths.root + "/config.toml", "--account", "personal"],
        environment: [:]
    )
    #expect(pruneResult.exitCode == GmailGatewayExitCode.success.rawValue)

    let validateResult = GmailGatewayCLI().run(
        arguments: ["config", "validate", "--config", paths.root + "/config.toml"],
        environment: [:]
    )
    #expect(validateResult.exitCode == GmailGatewayExitCode.configurationError.rawValue)
...
@Test func missingDefaultConfigMarksFallbackAccount() throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("gmail-gateway-default-tests-\(UUID().uuidString)", isDirectory: true)
    defer {
    ...
    let account = try #require(config.accounts.first)
    let validation = try GmailGatewayConfigLoader.validateConfig(environment: environment)
    let projected = try #require(GmailGatewayService(config: config).graphQLAccounts(sendEnabled: true).first)
...
@Test func fallbackAccountCannotSendEvenWithReadSendCredential() throws {
    ...
    let error = try requireGmailGatewayError {
        _ = try GmailGatewayWriteService(config: config).sendMessage(input: input, mode: .directSend)
    }
